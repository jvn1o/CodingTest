/*
    ...
    자동차 대여 기록 정보를 담은 CAR_RENTAL_COMPANY_RENTAL_HISTORY 테이블과 자
    동차 종류 별 대여 기간 종류 별 할인 정책 정보를 담은 CAR_RENTAL_COMPANY_DISCOUNT_PLAN 테이블 입니다.

    CAR_RENTAL_COMPANY_CAR 테이블은 아래와 같은 구조로 되어있으며,
    CAR_ID, CAR_TYPE, DAILY_FEE, OPTIONS 는 각각
    ...
    PLAN_ID, CAR_TYPE, DURATION_TYPE, DISCOUNT_RATE 는 각각
    ...
    자동차 ID, 자동차 종류, 대여 금액(컬럼명: FEE) 리스트를 출력하는 SQL 문을 작성해주세요.
...
/*
    SELECT
        C.CAR_ID,
        C.CAR_TYPE,
        FLOOR(C.DAILY_FEE * 30 * (1 - NVL(D.DISCOUNT_RATE, 0) / 100)) AS FEE
    FROM
    ...
    WHERE
        ...
        AND FLOOR(C.DAILY_FEE * 30 * (1 - NVL(D.DISCOUNT_RATE, 0) / 100)) BETWEEN 500000 AND 1999999
    ORDER BY
        FEE DESC,
        C.CAR_TYPE ASC,
        C.CAR_ID DESC;
