/*
    ...
    대여 기록 ID와 대여 금액 리스트를 출력하는 SQL 문을 작성해주세요.
...
/*
    SELECT
        h.HISTORY_ID,
        FLOOR(c.DAILY_FEE * rental_days * (1 - NVL(d.DISCOUNT_RATE, 0) / 100)) AS FEE
    FROM (
        SELECT
            h.HISTORY_ID,
            h.CAR_ID,
    ...
    LEFT JOIN CAR_RENTAL_COMPANY_DISCOUNT_PLAN d
        ...
        AND (
            (h.rental_days >= 7 AND h.rental_days < 30 AND d.DURATION_TYPE = '7일 이상') OR
            (h.rental_days >= 30 AND h.rental_days < 90 AND d.DURATION_TYPE = '30일 이상') OR
            (h.rental_days >= 90 AND d.DURATION_TYPE = '90일 이상')
        )
    WHERE c.CAR_TYPE = '트럭'
